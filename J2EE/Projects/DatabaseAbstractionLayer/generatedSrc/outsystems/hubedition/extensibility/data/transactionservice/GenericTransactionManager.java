...
public class GenericTransactionManager implements ITransactionManager {
    ...
    protected static class TransactionInfo {
        ...
        private ADOTransaction _trans;
        private boolean _free;
        private boolean _isReleasable;
        private boolean _isPoolable;
        private DateTime _creationTime;
        private DateTime _lastChange;
        private List<OSResultSet> _readers = new ArrayList<OSResultSet>();
        ...
        public final int getReaderCount()
        {
            ...
            {
                return LinqMethods.count(TypeInformation.get(OSResultSet.class), LinqMethods.where(_readers, 
    new Func.Func1<OSResultSet, Boolean>() {
        protected Boolean executeImpl(OSResultSet r) throws Exception
        {
            return !r.isClosed();
        }
    }));
        ...
        public final void addReader(OSResultSet reader) {
            synchronized(_readers)
            {
                ...
                CollectionsExtensions.apply(TypeInformation.get(OSResultSet.class), Arrays.asList(LinqMethods.toArray(TypeInformation.get(OSResultSet.class), LinqMethods.where(_readers, 
    new Func.Func1<OSResultSet, Boolean>() {
        protected Boolean executeImpl(OSResultSet r) throws Exception
        {
            return r.isClosed();
                    ...
                    new Action.Action1<OSResultSet>() {
                        protected void executeImpl(OSResultSet r) throws Exception
                        {
                            _readers.remove(r);
                        }
                    });
	 ...
	 *	@return	A commitable transaction managed by the user.
	 */
 ...
 *	@throws	InvalidTransactionReleaseException	
 */
...
/**
 *	Commits a transaction. The transaction is commited, the connection is closed and put back into the pool.
 ...
 *	@param	commit	If True, all transaction are commited. Otherwise, they are rolled back.
 *	@param	toFreeResources	If True, all resources are released and connections returned to the pool.
 */
protected void endTransaction(TransactionInfo transInfo, boolean commit, boolean toFreeResources) throws SQLException, UnsupportedOperationException {
    getTransactionPool().remove(transInfo.getTransaction());
    transInfo.release();
    try {
        if (commit)
